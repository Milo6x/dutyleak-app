...
-- Classifications table doesn't have workspace_id, it's linked via product_id
...
-- Savings ledger table doesn't have workspace_id, it's linked via product_id
...
-- Create RLS policies
CREATE POLICY classifications_workspace_isolation ON classifications
    USING (product_id IN (
...
CREATE POLICY duty_rates_workspace_isolation ON duty_rates
    USING (classification_id IN (
        SELECT id FROM classifications WHERE product_id IN (
            SELECT id FROM products WHERE workspace_id IN (
                SELECT workspace_id FROM workspace_users WHERE user_id = auth.uid()
            )
        )
...
CREATE POLICY duty_calculations_workspace_isolation ON duty_calculations
    USING (product_id IN (
        SELECT id FROM products WHERE workspace_id IN (
            SELECT workspace_id FROM workspace_users WHERE user_id = auth.uid()
        )
...
CREATE POLICY savings_ledger_workspace_isolation ON savings_ledger
    USING (product_id IN (
        SELECT id FROM products WHERE workspace_id IN (
            SELECT workspace_id FROM workspace_users WHERE user_id = auth.uid()
        )
